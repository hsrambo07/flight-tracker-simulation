...
function createPlane(flight) {
  ...
  plane.addEventListener('click', function(event) {
    ...
    // DO NOT create pulsing highlight automatically - causes white markers
...
function highlightPlane(flightId) {
  ...
  if (plane) {
    ...
    // DO NOT create pulsing highlight - causes unwanted white markers
    // createPulsingHighlight(plane, flight);
  }
}
...
function createPulsingHighlight(plane, flight) {
  ...
  let highlightColor = 'rgba(0, 0, 0, 0)'; // TRANSPARENT fallback instead of white
  let shadowColor = 'rgba(0, 0, 0, 0)'; // TRANSPARENT fallback shadow
...
function animateFlights() {
  ...
  flightData.forEach(flight => {
    ...
    // REALISTIC LANDED FLIGHT MANAGEMENT
    if (adjustedProgress >= 1) {
      ...
      // For flights that have been processed, decide what happens next
      const flightFate = Math.random();
      
      if (flight.isReturnFlight) {
        // Return flights - 70% chance to be removed, 30% to recycle as new flight
        if (flightFate < 0.7) {
          // REMOVE this return flight from the system
          removeFlightFromSystem(flight);
          console.log(`🗑️ Return flight ${flight.callsign} removed from system`);
          return;
        } else {
          // Recycle as new flight with new random route
          console.log(`♻️ Return flight ${flight.callsign} recycling as new flight`);
          recycleFlightAsNew(flight, now);
        }
      } else {
        // Original flights - 40% remove, 30% return flight (handled elsewhere), 30% recycle
        if (flightFate < 0.4) {
          // REMOVE this flight from the system
          removeFlightFromSystem(flight);
          console.log(`🗑️ Flight ${flight.callsign} removed from system after landing`);
          return;
        } else if (flightFate < 0.7) {
          // Will become return flight (handled by processCompletedFlights)
          // Keep flight in limbo until return flight is created
          flight.actualStartTime = now + 999999999; // Far future to keep it invisible
          return;
        } else {
          // Recycle as new flight with new random route
          console.log(`♻️ Flight ${flight.callsign} recycling as new flight`);
          recycleFlightAsNew(flight, now);
        }
      }
    }
...
// FLIGHT LIFECYCLE MANAGEMENT FUNCTIONS

// Remove a flight completely from the system
function removeFlightFromSystem(flight) {
  // Remove from main flight data array
  ...
  delete planeDivs[flight.id];
  
  // Clean up Leaflet markers
  const leafletMarker = leafletMarkers[flight.id];
  if (leafletMarker && leafletMap) {
    leafletMap.removeLayer(leafletMarker);
  }
  delete leafletMarkers[flight.id];
  
  // Clean up path canvases
  const pathCanvas = pathCanvases[flight.id];
  if (pathCanvas && pathCanvas.parentNode) {
    pathCanvas.parentNode.removeChild(pathCanvas);
  }
  delete pathCanvases[flight.id];
  
  // Remove from completed flights tracking
  completedFlightIds.delete(flight.id);
  
  console.log(`✈️ Removed flight ${flight.callsign} (${flight.id}) from system`);
}

// Recycle a flight with completely new random route
function recycleFlightAsNew(flight, now) {
  // Generate new random route - reuse the route generation logic
  const airlines = ['AI', 'IX', 'SG', 'UK', 'I5', 'G8', '6E', 'QP'];
  ...
  let origin = allAirports[Math.floor(Math.random() * allAirports.length)];
  let destination;
  do {
    destination = allAirports[Math.floor(Math.random() * allAirports.length)];
  } while (destination.code === origin.code);
  
  // Generate new airline and flight number
  const airline = airlines[Math.floor(Math.random() * airlines.length)];
  const flightNumber = Math.floor(Math.random() * 9000) + 1000;
  
  // Create new flight using the createFlight function
  const newFlightData = createFlight(origin, destination, flight.color, flight.id, airline, flightNumber);
  
  // Update existing flight object with new data
  Object.assign(flight, newFlightData);
  
  // Reset tracking
  flight.progress = getRandomFlightProgress();
  flight.lastProgress = undefined;
  flight.actualStartTime = now + Math.random() * 120000; // Start in 0-2 minutes
  flight.isReturnFlight = false; // Reset return flight status
  ...
  console.log(`♻️ Recycled flight as ${flight.callsign}: ${flight.origin} → ${flight.destination}`);
}

// Create animated radar waves around an aircraft
function createRadarWaves(plane, flight) {
  // DISABLED - No radar waves should appear unless explicitly following a flight
  return;
